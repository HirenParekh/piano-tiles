...
1.  **Melody Canvas & Bass Accompaniment**: The original PT2 game uses multiple array tracks `scores[0]` and `scores[1]`. Both tracks produce tappable tiles in the original game. In our engine, we use *Melody Canvas priority*. Melody notes always become tappable tiles. Bass notes that fall exactly within a melody tile's lifespan become "accompaniment dots" on that tile. However, bass notes that fall during melody rests become their own tappable tiles!
